...
:root{{
  ...
  /* ── Les crans d'AFFICHAGE ────────────────────────────────────────────
     ✏️ David, 2026-08-27 : « c'est encore disproportionné, le nom de MC est plus
     gros que sa citation ». Mesuré : le nom sortait à 46 px, la phrase à 38.

     🐛 Ma passe précédente n'avait rien changé ici, et pour une raison qui compte :
     j'avais rationalisé les crans de TEXTE en laissant quatre crans d'AFFICHAGE
     (46, 38, 34, 28) tels qu'ils étaient depuis la première version. Pire, le
     vérificateur les autorisait explicitement, parce qu'ils vivent en `clamp()`.
     J'avais créé une zone que le contrôle ne regardait pas.

     🔴 ET L'ORDRE N'ÉTAIT PAS DÉCIDÉ, il était hérité. Sur une carte de visite,
     le nom domine. Sur cette page, ce qui doit rester dans la tête d'un acheteur
     est la PHRASE, pas le nom : il a le nom partout ailleurs, dans les boutons,
     dans le pied, dans la signature. La phrase, il ne l'a qu'une fois. */
  /* 🐛 Troisième remarque de David sur la même chose, capture d'iPhone à l'appui :
     « encore disproportionné ». La cause n'était plus l'ordre, il était bon, mais
     le NOMBRE DE CRANS À L'ÉTROIT. Sur un écran de 390 px, la page servait six
     tailles entre 30 et 15 px : 30, 28, 26, 20, 17, 15. Deux crans séparés de
     deux pixels ne créent aucune hiérarchie, ils créent du bruit, et c'est
     exactement le défaut que ce même fichier corrigeait le matin sur les petites
     tailles. Je l'avais reproduit dans le haut de l'échelle.

     🔴 Sur téléphone, la page ne sert plus que QUATRE tailles : 26, 20, 17, 15,
     exactement celles de Mon Espace, qui est un produit conçu pour le téléphone.
     Les trois crans d'affichage s'y REJOIGNENT à 20 px, et c'est voulu : la
     couleur et la graisse continuent de les séparer là où la taille ne le peut
     plus. La phrase d'accroche descend à 26 pour la même raison, elle tenait sur
     quatre lignes à 30 px et une accroche sur quatre lignes ne s'accroche plus,
     elle se lit.

     🔴 Ils ne se séparent qu'en montant vers l'écran large,
     là où il y a
     la place pour les distinguer. Un clamp n'est pas qu'une taille qui rétrécit,
     c'est le nombre de crans lisibles qui change avec la largeur. */
  --t-affiche: clamp(1.625rem, 3.4vw, 2.625rem);  /* 26-42 px  ce qui doit rester */
  --t-nom: clamp(1.25rem, 2.8vw, 2.125rem);       /* 20-34 px  qui parle, un jalon */
  --t-titre: clamp(1.25rem, 2vw, 1.625rem);       /* 20-26 px  le nom d'une section */
...
.retenir p.phrase{{
  font-size:var(--t-affiche); font-weight:700; line-height:1.18;
  letter-spacing:-.015em; margin-bottom:16px;
}}
/* 🐛 Mesuré dans Chrome, pas déduit : l'accroche tenait sur QUATRE lignes sur un
   téléphone, même après être descendue à 26 px. La cause n'était pas la taille
   mais le `<br>` du contenu, écrit pour couper la phrase en deux sur un écran
   large. Sur 390 px, chacune de ses deux moitiés se recassait, et une accroche
   sur quatre lignes ne s'accroche plus, elle se lit.
   Le saut forcé est neutralisé sous 640 px : la phrase se coupe où la largeur le
   décide, ce qui la ramène à trois lignes. */
@media(max-width:640px){{.retenir p.phrase br{{display:none}}}}
...
/* 🐛 La signature était montée à 20 px gras le matin, en réponse à « c'est trop
   petit » quand elle valait 12. Entre les deux il y avait 15 et 17, et à 20 px
   gras vert nuit sur un téléphone elle rivalisait avec la phrase qu'elle signe.
   Elle redescend au corps : le gras, le vert nuit et le filet corail la
   distinguent déjà de l'appui, qui est en gris et sans graisse. */
.signature{{
  font-size:var(--t-corps); font-weight:700; color:var(--vert); margin-top:20px;
