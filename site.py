...
DEMANDES = [
    ...
    # ✏️ MC, 2026-08-28 : ses mots, en liste. Le titre change avec eux, parce
    # que « De la formation sur mesure » ne couvrait plus ce qu'il annonce : sur
    # les trois lignes, une seule est une formation.
    #
    # 🔴 Une carte qui porte une liste là où les deux autres portent une phrase
    # ne casse pas la rangée : les trois gardent la même largeur et le même
    # départ. C'est la seule des trois qui contient plusieurs choses, et
    # l'aplatir en phrase à rallonge la rendrait plus longue à lire, pas plus
    # courte.
    #
    # 🟡 Deux réserves de MC restent ouvertes, et la page les tranche par le
    # silence plutôt que par une invention. Elle a écrit « coaching individuel
    # de carrière » ; le wiki (`Offres/Coaching-individuel.md`) le positionne en
    # soutien ciblé aux participantes, pas en coaching de carrière. La mention
    # de carrière attend sa confirmation. Elle a aussi écrit « co-développement
    # en entreprise » ; le wiki ne le connaît que comme composante DANS le
    # parcours Essentielle, jamais comme offre autonome. Une quatrième ligne
    # s'ajoutera le jour où elle confirme qu'elle le vend seul.
    ("De l’accompagnement en entreprise",
     ["Parcours d’accompagnement pour les femmes",
...
/* ✏️ David, 2026-08-28 : « on va réduire un peu le logo de Momenta en haut ».
   26 → 22 px, et « un peu » est pris au mot : c'est le cran en dessous, pas une
   refonte. Le logo n'est pas le sujet de la page, c'est Marie-Claude ; à 22 px
   il passe sous la hauteur des lettres de son nom et cesse de lui disputer
   l'entrée. Il reste à 7 px du logo du pied, l'écart qui dit lequel des deux
   ouvre et lequel signe. */
header .logo{height:22px; display:block; margin-bottom:28px}
...
/* ✏️ David, 2026-08-28 : les glyphes des cartes s'en vont. Ils étaient au-dessus
   des titres, un par demande. Essayés, rendus, regardés : trois silhouettes qui
   se distinguaient bien, mais qui redisaient le titre sous elles sans rien y
   ajouter, et qui mettaient trois accents corail de plus dans une page qui en
   tient deux pour cent. La rangée est plus calme sans eux. */
.carte h3{color:var(--corail); line-height:1.3}
.carte p{font-size:var(--t-corps); color:var(--vert2); margin:0}
/* 🔴 Pas de puce ronde héritée du navigateur : elle arrive en gris système,
   à sa propre taille, et jure avec un kit qui ne cerne et ne pointe rien. Un
   filet corail de 6 px tient le rôle, dans l'accent que la carte porte déjà.
   🔴 Les lignes gardent l'interligne du corps : c'est une énumération à lire,
   pas un menu à cliquer, et les tasser en ferait une étiquette. */
.carte ul{margin:0; padding:0; list-style:none; color:var(--vert2)}
.carte li{position:relative; padding-left:16px}
.carte li+li{margin-top:8px}
.carte li::before{content:""; position:absolute; left:0; top:.62em;
  width:6px; height:2px; background:var(--corail)}
...
/* ✏️ David, 2026-08-28 : « on va rapetisser beaucoup le logo en pied de page ».
   ...
   reste plus grand, et l'écart entre les deux dit lequel des deux compte. */
...
def rendre():
    def corps(d):
        """Une phrase, ou plusieurs choses qu'on parcourt."""
        if isinstance(d, str):
            return f"<p>{e(d)}</p>"
        return "<ul>" + "".join(f"<li>{e(x)}</li>" for x in d) + "</ul>"

    demandes = "".join(
        f'<div class="carte"><h3>{e(t)}</h3>{corps(d)}</div>'
        for t, d in DEMANDES)
